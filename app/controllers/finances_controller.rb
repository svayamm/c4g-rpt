class FinancesController < ApplicationController
  ...
  def new
    @finance = Finance.new
    @finance.farm_id = params[:farm_id] unless params[:farm_id].nil?
    @finance.crop_id = params[:crop_id] unless params[:crop_id].nil?
  end
  ...
  def update
    respond_to do |format|
      if @finance.update(finance_params)
        format.html { redirect_to finances_path, notice: 'Finance was successfully updated.' }
